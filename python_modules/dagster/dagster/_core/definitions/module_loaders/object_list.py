        ...
        for dagster_def in dagster_def_list.loaded_defs:
            if isinstance(dagster_def, AssetsDefinition):
                new_asset = dagster_def.map_asset_specs(
                    _spec_mapper_disallow_group_override(group_name, automation_condition, dont_override_automation_condition)
                ).with_attributes(
...
def _spec_mapper_disallow_group_override(
    group_name: Optional[str], automation_condition: Optional[AutomationCondition], dont_override_automation_condition: Optional[bool]
    ...
    def _inner(spec: AssetSpec) -> AssetSpec:
        ...
        return spec.replace_attributes(
            ...
            automation_condition=spec.automation_condition if spec.automation_condition and dont_override_automation_condition else automation_condition if automation_condition else ...,
        )
